...
Page({
    data: {
        ...
        proGroup: [], //产品组
        gxGroup: [
            { "name": '广西新车', 'list': ['A1', 'A2', 'A3', 'A4', 'SanMa', 'HengXinA', 'Special'] },
        ...
        gx: [
            ...
            { "name": 'A3', "date": '36', 'list': ['16.07%', '17.71%', '18.26%', '19.36%', '20.45%', '21.55%'] },
    ...
    oninput: function(ev) {
        ...
        if (tag == 'chejia') {
            ...
            if (selectedTheftSchemeId === '0' || selectedTheftSchemeId === '1') {
                let theftAcountList = that.getTheftAcountList(selectedTheftSchemeId);
                that.setData({
                    theftAcountList: theftAcountList
                });
            }
            that.reset();
        }else if (tag == 'manualInputMoney') {
            let manualInputMoney = parseInt(val);
            that.setData({
                manualInputMoney: isNaN(manualInputMoney) ? '' : manualInputMoney
            });
            that.reset();
        }
    ...
    theftSchemeChange: (e) => {
        ...
        if (selectedTheftSchemeId === '0' || selectedTheftSchemeId === '1') {
            theftAcountList = that.getTheftAcountList(selectedTheftSchemeId);
        }
        that.setData({
     ...
     * @Author   huangz
     * @DateTime 2019-04-28
     * @param    {[type]}   selectedTheftSchemeId [description]
     * @return   {[type]}                         [description]
     */
    getTheftAcountList: (selectedTheftSchemeId) => {
        ...
        return [1200, 1500, percentTheft, '手动输入'];
    },
    //提示
    showModal: (content) => {
        wx.showModal({
    ...
    onCalu: () => {
        ...
        let manualInput = that.data.manualInput;//是否手动输入
        let manualInputMoney = parseInt(that.data.manualInputMoney);//手动输入金额
        ...
        if (selectedTheftSchemeId === '0' || selectedTheftSchemeId === '1') {
            if (manualInput===false &&parseInt(theftAcount) == 0) {
                that.showModal('请选择盗抢险金额');
                return false;
            }
            if (manualInput===true &&(isNaN(manualInputMoney)||manualInputMoney == 0)) {
                that.showModal('请输入盗抢险金额');
                return false;
            }

            if (manualInput===true && manualInputMoney>0) {
               theftAcount = manualInputMoney
            }
